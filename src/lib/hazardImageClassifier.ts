...
async function tryBackendHuggingFaceInspection(
  ...
  try {
    ...
    return {
      ...
      decision: isRelevant ? 'sent_to_admin_for_manual_inspection' : 'rejected',
      message: data.summaryMessage || (isRelevant ? 'Possible hazard environment detected. Awaiting official verification.' : 'IRRELEVANT / MANUAL REVIEW REQUIRED'),
