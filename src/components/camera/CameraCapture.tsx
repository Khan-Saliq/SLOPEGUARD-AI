...
import { Camera, Video, X, Check, AlertCircle, RotateCcw, Loader2 } from 'lucide-react';
import { Button } from '../ui/Button';

interface CameraCaptureProps {
  onCapture: (data: { blob: Blob; url: string; type: 'image' | 'video'; timestamp: Date; metadata: CaptureMetadata }) => void;
  onCancel: () => void;
  mode: 'photo' | 'video';
}
...
export function CameraCapture({ onCapture, onCancel, mode }: CameraCaptureProps) {
  ...
  const startCamera = async () => {
    try {
      ...
      if (error.name === 'NotAllowedError' || error.name === 'PermissionDeniedError') {
        ...
        setStatus('error');
        setErrorMessage('No camera found on this device. Please connect a camera or use a device with a built-in camera.');
  ...
  const capturePhoto = () => {
    ...
    canvas.toBlob((blob) => {
      ...
      // Photo will be sent with metadata on confirmation
    }, 'image/jpeg', 0.92);
  };
  ...
  return (
    ...
    >
      <div className="relative w-full h-[400px] bg-black rounded-xl overflow-hidden border-2 border-accent-bright/50">
        ...
        {/* Recording indicator */}
        {isRecording && (
          <div className="absolute top-4 left-4 flex items-center gap-2 bg-red-500 text-white px-3 py-1.5 rounded-full font-mono text-sm">
            ...
            <div className="flex items-center gap-3">
              {mode === 'photo' ? (
                <Button
                  ...
                  className="h-16 w-16 rounded-full bg-white hover:bg-gray-200 flex items-center justify-center"
                >
      ...
      <div className="mt-2 flex items-center justify-center gap-2 text-xs text-emerald-400">
        ...
        <span>Camera Active - Captured via Browser API</span>
      </div>
    </motion.div>
  );
}
